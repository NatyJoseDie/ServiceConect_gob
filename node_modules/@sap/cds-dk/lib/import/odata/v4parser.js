...
function _generateCSN(edmxAsJson, ignorePersistenceSkip, parserContext, importerContext) {
    ...
    for (const entity in csn.definitions) {
        if (entity.type !== 'entity') continue;

        const containingCompositions = Object.keys(csn.definitions[entity].elements).filter(e => 
            csn.definitions[entity].elements[e].type === 'cds.Composition' && 
            csn.definitions[entity].elements[e].on
        );
        for (const comp of containingCompositions) {
            const compTarget = csn.definitions[csn.definitions[entity].elements[comp].target];
            const backLink = Object.keys(compTarget.elements).find(e => 
                compTarget.elements[e].type === 'cds.Association' && 
                compTarget.elements[e].target === entity
            )
